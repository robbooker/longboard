...
export const CHAT_FILE_TYPES = ['application/pdf', 'image/jpeg', 'image/png', 'image/gif'] as const;
export type ChatFileType = typeof CHAT_FILE_TYPES[number];
export type ChatAttachmentScope = {room: string};
export type ChatAttachment = {id: string; filename: string; byte_size: number; mime_type: ChatFileType};
const extensions: Record<ChatFileType, string[]> = {'application/pdf':['pdf'],'image/jpeg':['jpg','jpeg'],'image/png':['png'],'image/gif':['gif']};
export function attachmentMetadata(filename: unknown, mime: unknown, size: unknown) {
  ...
  if (!CHAT_FILE_TYPES.includes(mime as ChatFileType) || !extensions[mime as ChatFileType].includes(filename.split('.').at(-1)?.toLowerCase() ?? '')) throw Error('Choose a PDF, JPEG, PNG or GIF file.');
