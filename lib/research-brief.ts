// Per-ticker research brief assembly. Originally lived inline in
// app/api/research/route.ts; extracted so the Strategies morning
// routine (lib/strategies/long-short/morning-run.ts) can call it
// directly without spinning up an HTTP boundary. The existing
// /api/research GET handler re-exports this as its implementation.
...
async function fetchFundamentals(ticker: string): Promise<Fundamentals> {
  ...
  if (!searchRes.ok) {
    return fetchFundamentalsFallback(ticker);
  }
  ...
  return fetchEdgarByCik(cik.toString(), ticker);
}

async function fetchFundamentalsFallback(ticker: string): Promise<Fundamentals> {
  ...
  const match = Object.values(tickersData).find(
    (entry: unknown) => (entry as { ticker?: string }).ticker?.toUpperCase() === ticker.toUpperCase()
  ) as { cik_str: number } | undefined;

  if (!match) throw new Error(`No EDGAR CIK found for ${ticker}`);

  return fetchEdgarByCik(match.cik_str.toString(), ticker);
}
...
function getLatestFact(
  ...
  const f = facts as { "us-gaap"?: Record<string, { units?: { USD?: unknown[] } }> };
  const fact = f?.["us-gaap"]?.[concept];
  if (!fact) return null;
  const entries = fact.units?.USD;
  if (!entries || entries.length === 0) return null;
  const sorted = [...entries]
    .filter((e: unknown) => {
      const form = (e as { form?: string }).form;
      return form === "10-K" || form === "10-Q" || form === "20-F";
    })
    .sort((a: unknown, b: unknown) => {
      const ae = (a as { end?: string }).end ?? "";
      const be = (b as { end?: string }).end ?? "";
      return be.localeCompare(ae);
    });
  return sorted.length > 0 ? (sorted[0] as { val: number }).val : null;
}
...
async function fetchEdgarByCik(
  ...
  // Ticker name is retained for trace logging in a future phase; not used here.
  void ticker;
...
async function fetchNews(ticker: string): Promise<NewsData> {
  ...
  if (exaResult.status === "fulfilled" && (exaResult.value as { results?: unknown[] })?.results) {
    const value = exaResult.value as { results: Array<{ title?: string; url?: string; highlights?: string[] }> };
    results.exaResults = value.results.map((r) => ({
      title: r.title ?? "",
      url: r.url ?? "",
  ...
  if (pplxResult.status === "fulfilled" && pplxResult.value) {
    const value = pplxResult.value as { choices?: Array<{ message?: { content?: string } }> };
    results.perplexitySummary = value.choices?.[0]?.message?.content ?? null;
  }
...
/** Assembles the full ResearchBrief for a ticker. Runs all three layers
 *  in parallel; partial-success semantics — a failure in any layer is
 *  captured in `errors` rather than thrown. The caller decides whether
 *  to accept a partial brief (run-daily accepts, the strategies
 *  morning-run drill-in tool accepts). */
export async function fetchResearchBrief(ticker: string): Promise<ResearchBrief> {
  const upper = ticker.toUpperCase().trim();
  const errors: string[] = [];

  const [marketResult, fundResult, newsResult] = await Promise.allSettled([
    fetchMarketData(upper),
  ...
  if (marketResult.status === "rejected") {
    errors.push(`Market data: ${(marketResult.reason as Error)?.message ?? "failed"}`);
  }
  if (fundResult.status === "rejected") {
    errors.push(`Fundamentals: ${(fundResult.reason as Error)?.message ?? "failed"}`);
  }
  if (newsResult.status === "rejected") {
    errors.push(`News: ${(newsResult.reason as Error)?.message ?? "failed"}`);
  }
