import type { MorningEmailDraft, MorningEmailStock } from "./types";
...
function suggestedLevels(stock: MorningEmailStock): Levels {
  const last = stock.last;
  const t1Pct = 0.20;
  const t2Pct = 0.40;
  const stopPct = 0.10;
  const target1 = last * (1 + t1Pct);
  const target2 = last * (1 + t2Pct);
  const stopLoss = last * (1 - stopPct);
  const upside = target1 - last;
  const downside = last - stopLoss;
  const rr = downside > 0 ? upside / downside : 0;
  return {
    entryLow: formatPrice(last * 0.97),
    entryHigh: formatPrice(last * 1.01),
    target1: formatPrice(target1),
    target2: formatPrice(target2),
    stopLoss: formatPrice(stopLoss),
    rr: rr > 0 ? `1 : ${rr.toFixed(1)}` : "—",
    entryNote: "Pullback into base",
    target1Note: `+${(t1Pct * 100).toFixed(0)}% from current`,
    target2Note: `+${(t2Pct * 100).toFixed(0)}% extension`,
    stopNote: `−${(stopPct * 100).toFixed(0)}% from current`,
  };
}
...
function renderTopPick(stock: MorningEmailStock): string {
  const levels = suggestedLevels(stock);
          ...
          <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="background:${COLORS.ink};">
            <tr>
              <td style="padding:24px 28px 8px;font-family:${FONT_SANS};">
                <div style="font-family:${FONT_MONO};font-size:10px;letter-spacing:1.8px;color:${COLORS.gold};font-weight:700;margin-bottom:6px;">PRICE TARGETS &amp; LEVELS</div>
                <div style="font-family:${FONT_SANS};font-size:22px;font-weight:800;color:${COLORS.paper};letter-spacing:-0.6px;line-height:1.2;">Defined risk. <span style="font-family:${FONT_SERIF};font-style:italic;font-weight:500;color:${COLORS.gold};">Real upside.</span></div>
              </td>
            </tr>
            <tr>
              <td style="padding:18px 28px 26px;">
                <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" style="font-family:${FONT_MONO};">
                  ${levelRow("ENTRY ZONE", `${levels.entryLow} – ${levels.entryHigh}`, levels.entryNote, false, false)}
                  ${levelRow("TARGET 1", levels.target1, levels.target1Note, true, false)}
                  ${levelRow("TARGET 2", levels.target2, levels.target2Note, true, false)}
                  ${levelRow("STOP LOSS", levels.stopLoss, levels.stopNote, false, false)}
                  ${levelRow("R / R RATIO", levels.rr, "Defined risk, real upside", false, true)}
                </table>
                <div style="margin-top:14px;font-family:${FONT_SERIF};font-style:italic;font-size:12px;color:rgba(244,241,232,0.55);line-height:1.4;">
                  Auto-generated reference levels from current price. Not analyst targets.
                </div>
              </td>
            </tr>
          </table>
...
function levelRow(label: string, value: string, note: string, accent: boolean, isLast: boolean): string {
  const labelColor = accent ? COLORS.gold : "rgba(244,241,232,0.6)";
  const valueColor = label === "R / R RATIO" ? COLORS.gold : COLORS.paper;
  const borderBottom = isLast ? "border-bottom:1px solid rgba(244,241,232,0.18);" : "";
  return `<tr class="level-row">
  <td style="padding:14px 0;border-top:1px solid rgba(244,241,232,0.18);${borderBottom}">
    <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
        <td width="110" style="font-family:${FONT_MONO};font-size:11px;letter-spacing:1.4px;color:${labelColor};font-weight:700;">${escapeHtml(label)}</td>
        <td style="font-family:${FONT_SANS};font-size:18px;font-weight:800;color:${valueColor};letter-spacing:-0.5px;">${escapeHtml(value)}</td>
        <td class="level-note" align="right" style="font-family:${FONT_SERIF};font-style:italic;font-size:13px;color:rgba(244,241,232,0.6);">${escapeHtml(note)}</td>
      </tr>
    </table>
  </td>
</tr>`;
