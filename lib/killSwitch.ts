...
function getServiceRoleClient() {
  ...
  if (!url || !key) {
    throw new Error(
      "Kill switch helper missing NEXT_PUBLIC_SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY"
    );
  }
...
export async function isOrderSubmissionEnabled(): Promise<{
  enabled: boolean;
  reason?: KillSwitchReason;
}> {
  const state = await getKillSwitchState();
  if (state.envOverride) return { enabled: false, reason: "env_override" };
  if (!state.orderSubmissionEnabled)
    return { enabled: false, reason: "soft_disabled" };
  return { enabled: true };
}
