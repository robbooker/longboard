...
beforeEach(()=>{
 ...
 mock.admin.mockReturnValue({storage:{from:()=>({remove:mock.remove})},from:(table:string)=>{
  ...
  const q={select:()=>q,delete:()=>{removing=true;return q;},lt:()=>q,lte:()=>q,neq:()=>q,order:()=>q,
   limit:async()=>({error:null,data:table==='chat_attachments'?[{id:'orphan'}]:[{path:'quarantine/expired'},{path:'clean/deleted'}]}),
...
it('acknowledges cleanup only after storage removal succeeds',async()=>{
 const response=await GET(req());expect(response.status).toBe(200);expect(mock.remove).toHaveBeenCalledWith(['quarantine/expired','clean/deleted']);expect(mock.ack).toHaveBeenCalledWith(['quarantine/expired','clean/deleted']);
});
