...
vi.mock("@/lib/chatAdmin",()=>({createChatAdminClient:()=>({rpc:mock.budget})}));
...
beforeEach(()=>{vi.clearAllMocks();mock.auth.mockResolvedValue({ok:true,user:{id:"member"}});mock.rpc.mockResolvedValue({data:[],error:null});mock.budget.mockResolvedValue({data:true,error:null});mock.embed.mockResolvedValue({vectors:[[1,2]],tokens:2});});
