...
beforeEach(()=>{
 vi.clearAllMocks(); mocks.auth.mockResolvedValue({ok:true,user:{id:"account"}});mocks.member.mockResolvedValue({id:"member"});
...
it("requires a signed-in linked member",async()=>{
 ...
 mocks.auth.mockResolvedValue({ok:true,user:{id:"account"}});mocks.member.mockResolvedValue(null);expect((await GET(req())).status).toBe(403);
});
