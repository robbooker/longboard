...
it('derives the account and room access from the session',async()=>{
 ...
 expect(mocks.rpc).toHaveBeenCalledWith('chat_activity_inbox',{actor:id,rooms:['main','social']});
 await POST(req({kind:'all',actor:'forged',rooms:['shortscout'],mentionThrough:4,dmThrough:9}));
 expect(mocks.rpc).toHaveBeenLastCalledWith('read_chat_activity',{actor:id,rooms:['main','social'],mention_through:4,mention_id:null,dm_through:9,dm_conversation:null});
});
...
it('keeps read actions scoped to their type and selected item',async()=>{
 await POST(req({kind:'dm',id,dmThrough:3,mentionThrough:100}));expect(mocks.rpc).toHaveBeenLastCalledWith('read_chat_activity',{actor:id,rooms:['main','social'],mention_through:0,mention_id:null,dm_through:3,dm_conversation:id});
