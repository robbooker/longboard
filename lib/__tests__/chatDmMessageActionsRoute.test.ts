...
it.each([{target:'room-summaries'},{messageId:'not-a-uuid'},{expectedRevision:-1},{expectedRevision:1.5},{expectedRevision:null},{expectedRevision:2147483648},{body:'x'.repeat(2001)}])('rejects malformed mutation %j',async overrides=>{expect((await POST(req({...payload,...overrides}))).status).toBe(400);expect(mock.rpc).not.toHaveBeenCalled();});
...
it('delegates empty-caption validation to the message-aware RPC and returns a safe error for text-only messages',async()=>{mock.rpc.mockResolvedValue({error:{message:'invalid_message'}});expect((await POST(req({...payload,body:''}))).status).toBe(409);expect(mock.rpc).toHaveBeenCalledWith('longboard_chat_dm_message_action',expect.objectContaining({p_body:''}));});
it('routes attachment-only sends through the atomic media RPC with verified identity',async()=>{expect((await POST(req({action:'send',target,clientId:messageId,body:'',attachmentIds:[messageId],userId:target}))).status).toBe(200);expect(mock.rpc).toHaveBeenCalledWith('send_chat_dm_ack',expect.objectContaining({p_user_id:actor,p_files:[messageId],p_body:''}));});
it('rejects unbound media, duplicate IDs and request attachments before RPC',async()=>{for(const data of [{action:'send',attachmentIds:[messageId,messageId]},{action:'send',attachmentIds:['bad']},{action:'request',attachmentIds:[messageId]},{action:'send',attachmentIds:[]}])expect((await POST(req({target,clientId:messageId,body:'',...data}))).status).toBe(400);expect(mock.rpc).not.toHaveBeenCalled();});
