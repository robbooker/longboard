...
vi.mock('@/lib/chatAdmin',()=>({createChatAdminClient:()=>({from:m.from}),requestOriginAllowed:()=>true,readPublicRoomState:async()=>({isOpen:true})}));
...
beforeEach(()=>{
 ...
 m.insert.mockReturnValue({select:()=>({single:async()=>({data:{id:'reply',body:'Reply text'},error:null})})});
...
it('persists the parent and authenticated identity for replies',async()=>{
 expect((await POST(send(id))).status).toBe(200);expect(m.insert).toHaveBeenCalledWith({room_slug:'main',guest_id:'member',member_id:'member',author_label:'Trusted',body:'Reply text',reply_to_id:id});
});
