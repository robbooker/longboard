...
export async function attachmentAccess(db:SupabaseClient,auth:Extract<ChatAuthResult,{ok:true}>,scope:{room_slug?:string|null;conversation_id?:string|null},write=false){
 ...
 if(scope.conversation_id){
  ...
  if(write&&!canWriteChatRoom(auth.access,room))throw new AttachmentError(room==="gainers"?"Gainers is a read-only broadcast channel.":"Only admins can attach files in announcement and recording channels.",403);
