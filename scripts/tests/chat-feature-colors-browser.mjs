...
try {
 ...
 await page.goto(`http://localhost:3204/chat/features?request=${requests[3].id}`);await page.waitForSelector('nav[aria-label="Feature requests"] button[data-status="publish_failed"]');
 ...
 await page.screenshot({path:'/tmp/feature-colors-dark.png',fullPage:true});
 await page.click('button[aria-label="Light mode"]');assert.equal(await page.$eval('main',e=>e.dataset.featureTheme),'light');await page.screenshot({path:'/tmp/feature-colors-light.png',fullPage:true});
 ...
 await page.setViewport({width:390,height:844});await page.screenshot({path:'/tmp/feature-colors-mobile.png',fullPage:true});
 ...
 assert.deepEqual(errors,[]);console.log('PASS all ten distinct status colors, red ready, black/faded completed cards, selected contrast, status badge, light/dark, 320/390/768/1280 layouts and reduced-motion behavior.');
