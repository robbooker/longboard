...
await db.exec('reset role');
await db.exec(await readFile(new URL('../../supabase/migrations/20260916160122_chat_message_actions.sql',import.meta.url),'utf8'));
await db.exec('set role service_role');
const change=async(actor,message,room,action,body=null,expected=null,admin=false)=>(await rows('select change_chat_message($1,$2,$3,$4,$5,$6,$7) as m',[actor,message,room,action,body,expected,admin]))[0].m;
const editable=await post('social',ssMember.id);
const edited=await change(session.accountId,editable,'social','edit','updated','hello');
check(edited.body,'updated');check(!!edited.edited_at,true);
check((await rows('select embedding from longboard_chat_embeddings where message_id=$1',[editable]))[0].embedding,null);
await assert.rejects(()=>change(session.accountId,editable,'social','edit','lost update','hello'),/message_changed/);checks++;
await assert.rejects(()=>change(lb,editable,'social','edit','not mine','updated',true),/message_forbidden/);checks++;
await assert.rejects(()=>change(lb,editable,'social','delete',null,null,true),/message_forbidden/);checks++;
await assert.rejects(()=>change(session.accountId,lbMessage,'main','delete'),/room_forbidden/);checks++;
await change(session.accountId,editable,'social','delete');
check((await rows('select count(*)::int n from longboard_chat_embeddings where message_id=$1',[editable]))[0].n,0);
check((await rows('select action from chat_message_actions where message_id=$1 order by id',[editable])).map(r=>r.action),['edit','delete']);
await db.exec('reset role');await rows("update profiles set role='admin' where id=$1",[lb]);await db.exec('set role service_role');
const moderated=await post('social',ssMember.id);
await assert.rejects(()=>change(lb,moderated,'social','edit','admin rewrite','hello',true),/message_forbidden/);checks++;
await change(lb,moderated,'social','delete',null,null,true);
check((await rows('select action from chat_message_actions where message_id=$1',[moderated]))[0].action,'admin_delete');
await db.exec('reset role;set role authenticated');
await assert.rejects(()=>change(lb,lbMessage,'main','delete',null,null,true),/permission denied/);checks++;
await assert.rejects(()=>rows('select * from chat_message_actions'),/permission denied/);checks++;
