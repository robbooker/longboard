...
for(const status of ['in_progress','ready','done','blocked'])await assert.rejects(async()=>archive(await create(status)),/ticket_already_picked_up/);
await assert.rejects(async()=>archive(await create('approved',true)),/ticket_already_picked_up/);
const declined=await create('declined');await archive(declined);
await assert.rejects(async()=>archive(await create('declined',true)),/ticket_already_picked_up/);
const releaseOwned=await create('declined');
await db.query("insert into chat_feature_releases(request_id,pr_number,head_sha,version,state) values($1,999,$2,1,'ready')",[releaseOwned,'a'.repeat(40)]);
await assert.rejects(()=>archive(releaseOwned),/ticket_already_picked_up/);
console.log('PASS declined/release guards and  archive permissions, stale revisions, idempotency/history, audit metadata, archive-before-pickup, pickup-before-archive and protected work states.');
await db.close();
