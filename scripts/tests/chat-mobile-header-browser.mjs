...
try {
 ...
 const nav='nav[aria-label="Chat rooms"]',feature='button[aria-label^="Feature notifications"]',inbox='button[aria-haspopup="dialog"]';
 for(const width of [320,390,768,1099]) {
  ...
  await page.click(`${nav} ${inbox}`);await page.waitForSelector('dialog[open]',{visible:true});
  await page.click('button[aria-label="Close inbox"]');await page.waitForFunction(()=>document.activeElement?.getAttribute('aria-label')==='Open room navigation');
 ...
 await page.click(`${nav} ${inbox}`);await page.waitForSelector('dialog[open]',{visible:true});
 await page.setViewport({width:1440,height:900});await page.waitForSelector('section[aria-label="Private inbox"]',{visible:true});assert.ok(await page.$(`header ${feature}`));assert.equal(await page.$('dialog[open]'),null);
 await page.setViewport({width:390,height:844});await page.waitForSelector('dialog[open]',{visible:true});await page.click('button[aria-label="Close inbox"]');
 assert.deepEqual(errors,[]);console.log('PASS mobile header/nav placement, notifications panel bounds, inbox popup, close focus, desktop relocation, resize with inbox open, no overflow/runtime errors.');
