...
const target=new URL(`.chat-updates-fixture-${process.pid}.mjs`,import.meta.url);
let source=await readFile(new URL('chat-mobile-fixture.mjs',import.meta.url),'utf8');
source=source.replaceAll('54404','54465').replaceAll('3204','3265');
...
`;
source=source.replace("now()+($3 * interval '1 second')","now()-interval '1 hour'+($3 * interval '1 second')");
source=source.replace("function user(p)","await db.exec(await readFile(root+'/supabase/migrations/20260918140909_chat_reply_notifications.sql','utf8'));\nfunction user(p)");
