...
// SHORTSCOUT is gated in the database, including direct API and context reads.
await db.exec('reset role');
await db.exec(await readFile(new URL('../../supabase/migrations/20260915232125_chat_shortscout_admin_room.sql',import.meta.url),'utf8'));
await ok("update profiles set role='admin' where id=$1",[users[0]]);
await db.exec('set role service_role');
const shortMessage=(await ok("insert into longboard_chat_messages(room_slug,guest_id,member_id,author_label,body) values('shortscout',$1,$1,'Admin','Private scout fixture') returning id",[members[0].id]))[0].id;
await denied("insert into longboard_chat_messages(room_slug,guest_id,member_id,author_label,body) values('shortscout',$1,$1,'Member','Denied')",[members[1].id],/shortscout_admin_only/);
await denied("insert into longboard_chat_reactions(message_id,guest_id) values($1,$2)",[shortMessage,members[1].id],/shortscout_admin_only/);
await ok("insert into longboard_chat_reactions(message_id,guest_id) values($1,$2)",[shortMessage,members[0].id]);
assert.equal((await ok('select * from longboard_chat_embeddings where message_id=$1',[shortMessage])).length,0);checks++;
await db.exec('reset role; set role authenticated');
await ok("select set_config('request.jwt.claim.sub',$1,false)",[users[1]]);
assert.equal((await ok('select * from longboard_chat_messages where id=$1',[shortMessage])).length,0);checks++;
assert.equal((await ok('select * from longboard_chat_reactions where message_id=$1',[shortMessage])).length,0);checks++;
assert.equal((await ok('select * from longboard_chat_search_context($1)',[shortMessage])).length,0);checks++;
await ok("select set_config('request.jwt.claim.sub',$1,false)",[users[0]]);
assert.equal((await ok('select * from longboard_chat_messages where id=$1',[shortMessage])).length,1);checks++;
assert.equal((await ok('select * from longboard_chat_search_context($1)',[shortMessage])).length,1);checks++;
assert.equal((await ok("select * from search_longboard_chat('scout','all')")).length,0);checks++;
await db.exec('reset role');
await ok("update profiles set role='user' where id=$1",[users[0]]);
