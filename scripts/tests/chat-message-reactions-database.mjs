...
await react(1,roomId,'rob');await react(1,roomId,'rob');await react(0,roomId,'rob');
assert.deepEqual((await read(1,roomId)).find(r=>r.emoji==='rob'),{emoji:'rob',count:2,mine:true,names:['Member 0','Member 1']});checks++;
await react(1,roomId,'rob',false);assert.equal((await read(1,roomId)).find(r=>r.emoji==='rob').count,1);checks++;
...
await assert.rejects(()=>react(1,roomId,'rob'),/chat_paused/);await assert.rejects(()=>react(1,roomId,'like',false),/chat_paused/);checks+=2;
...
for(const emoji of ['like','heart','laugh','rob']){await react(1,message,emoji,true,null,c);await react(1,message,emoji,true,null,c);}
assert.equal((await read(1,message,null,c)).length,4);assert((await read(1,message,null,c)).every(r=>r.count===1&&r.mine));checks+=2;
await assert.rejects(()=>react(2,message,'rob',true,null,c),/conversation_not_found/);assert.deepEqual(await read(2,message,null,c),[]);checks+=2;
...
await assert.rejects(()=>react(1,message,'rob',false,null,c),/conversation_unavailable/);await assert.rejects(()=>react(1,roomId),/message_not_found/);checks+=2;
await act(users[0],'unblock',c);
await mutate(users[0],c,message,'delete',0);assert.deepEqual(await read(1,message,null,c),[]);await assert.rejects(()=>react(1,message,'rob',true,null,c),/message_not_found/);checks+=2;
