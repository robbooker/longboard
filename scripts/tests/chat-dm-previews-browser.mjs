...
try {
 ...
 const dm='section[aria-label="Private conversation"]';
 ...
 await page.reload({waitUntil:'domcontentloaded'});await page.waitForSelector('nav[aria-label="Chat rooms"] button[data-active]');await page.evaluate(()=>[...document.querySelectorAll('nav[aria-label="Chat rooms"] button[data-active]')].find(e=>e.textContent.includes('Bob')).click());await page.waitForSelector(`${dm} img[alt="Shared GIF"]`);
 ...
 const context=await browser.createBrowserContext();const bob=await context.newPage();await bob.goto(`${process.env.CHAT_TEST_URL||'http://localhost:3204'}/login?next=%2Fchat`);await bob.waitForSelector('#li-email');await bob.reload({waitUntil:'networkidle0'});await bob.type('#li-email','bob@example.test');await bob.type('#li-password','demo-only');await bob.click('button[type="submit"]');await bob.waitForSelector('textarea[aria-label="Message LB"]');
