...
const target=new URL(`.chat-s05-fixture-${process.pid}.mjs`,import.meta.url);
let source=await readFile(new URL('chat-mobile-fixture.mjs',import.meta.url),'utf8');
source=source.replaceAll('54404','54484').replaceAll('3204','3284');
...
source=source.replace("function user(p)","for(const file of ['20260918115352_chat_dm_directory.sql','20260918140909_chat_reply_notifications.sql','20260918141732_chat_ss_mastermind_access.sql','20260918160518_chat_admin_public_room_access.sql','20260918172508_chat_dm_sound_cursor.sql','20260918211138_chat_buddy_async_queue.sql','20260918211147_chat_s03_dm_ack.sql','20260919002508_chat_attachment_previews.sql'])await db.exec(await readFile(root+'/supabase/migrations/'+file,'utf8'));\nfunction user(p)");
source=source.replace("['chat_thread_counts','search_longboard_chat'","['longboard_chat_dm_directory','chat_thread_counts','search_longboard_chat'");
