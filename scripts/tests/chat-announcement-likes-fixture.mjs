...
console.log('Isolated chat fixture server on http://127.0.0.1:54414. Test users: alice@example.test, bob@example.test, mallory@example.test; password: demo-only');
...
await db.exec(await readFile(`${root}/supabase/migrations/20260917231038_chat_announcement_member_reactions.sql`,'utf8'));
...
createServer((req,res)=>{queue=queue.then(async()=>{
 res.setHeader('Access-Control-Allow-Origin','http://localhost:3214');res.setHeader('Access-Control-Allow-Headers','authorization,apikey,content-type,x-client-info,prefer,accept-profile,content-profile,x-supabase-api-version,x-upsert,cache-control');res.setHeader('Access-Control-Allow-Methods','GET,POST,PUT,PATCH,DELETE,OPTIONS');
 if(req.method==='OPTIONS'){res.writeHead(204);res.end();return;}
 const send=(v,status=200)=>{res.writeHead(status,{'content-type':'application/json'});res.end(JSON.stringify(v));};
 const url=new URL(req.url,'http://localhost');
 if(url.pathname==='/test/scout') {res.writeHead(302,{'Set-Cookie':`lb-chat-session=${scoutToken}; Path=/; HttpOnly; SameSite=Lax`,'Location':'http://localhost:3214/chat?room=shortscout&popout=1'});res.end();return;}
...
}).catch(e=>{console.error(e.message);res.end();});}).listen(54414,'127.0.0.1');
