...
try {
 ...
 await page.click(dmButton);await page.waitForSelector('section[aria-label="Private conversation"]',{visible:true});await page.waitForFunction(()=>document.querySelector('section[aria-label="Private conversation"]')?.textContent.includes('Hi Alice!'));
 ...
 const clickText=async(text,root='section[aria-label="Private conversation"]')=>{const handles=await page.$$(`${root} button`);for(const h of handles){if((await h.evaluate(e=>e.textContent))===text){await h.click();return;}}throw Error(`Missing button ${text}`);};
 await clickText('Accept request');await page.waitForSelector('#dm-body',{visible:true});
 await page.type('#dm-body','Desktop center DM test');await clickText('Send message');await page.waitForFunction(()=>document.querySelector('section[aria-label="Private conversation"]')?.textContent.includes('Desktop center DM test'));
 for(const width of [1100,1280,1440,1920]){
  await page.setViewport({width,height:1000});
  const g=await page.evaluate(()=>{const nav=document.querySelector('nav[aria-label="Chat rooms"]').getBoundingClientRect(),dm=document.querySelector('section[aria-label="Private conversation"]').getBoundingClientRect(),header=document.querySelector('h1').getBoundingClientRect();return {navRight:nav.right,dmLeft:dm.left,dmWidth:dm.width,headerBottom:header.bottom,dmTop:dm.top,overflow:document.documentElement.scrollWidth>innerWidth};});
 ...
 await page.setViewport({width:390,height:844});await page.waitForSelector('section[aria-label="Private conversation"] #dm-body',{visible:true});assert.equal(await page.$eval('#dm-body',e=>e.value),'Resize draft');
 await page.setViewport({width:1280,height:900});await page.waitForSelector('section[aria-label="Private conversation"] #dm-body',{visible:true});assert.equal(await page.$eval('#dm-body',e=>e.value),'Resize draft');
 await page.keyboard.press('Escape');await page.waitForSelector('textarea[aria-label="Message LB"]',{visible:true});
 for(const width of [320,390,768]){
  await page.setViewport({width,height:900});await page.waitForSelector('button[aria-label="Open room navigation"]',{visible:true});await page.click('button[aria-label="Open room navigation"]');await page.waitForSelector(dmButton,{visible:true});await page.click(dmButton);await page.waitForSelector('section[aria-label="Private conversation"] #dm-body',{visible:true});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.evaluate(()=>[...document.querySelectorAll('section[aria-label="Private conversation"] button')].find(e=>e.textContent==='Back to room').click());await page.waitForSelector('textarea[aria-label="Message LB"]',{visible:true});
 }
 assert.deepEqual(errors,[]);console.log('PASS sidebar unread cards, center DM request acceptance/send, room draft and room/search switching, desktop geometry, mobile navigation and main-area DM, resize draft retention, no page errors');
