-- Keep cancelled proposals/history, but never let the worker claim them.
alter table public.chat_feature_requests drop constraint chat_feature_requests_status_check;
alter table public.chat_feature_requests add constraint chat_feature_requests_status_check
 check(status in ('discussion','approved','in_progress','ready','declined','blocked','done','archived'));
alter table public.chat_feature_requests
 add column archived_at timestamptz,
 add column archived_by uuid references public.chat_accounts(id);

create function public.archive_chat_feature(actor uuid, feature uuid, expected_revision integer)
...
begin
 ...
 if r.status not in ('discussion','approved') or r.claimed_at is not null or r.worker_token is not null
