...
export default function MessageActions({message,room,own,admin,paused,onEdited,onDeleted}:{message:PublicChatMessage;room:ChatRoom;own:boolean;admin:boolean;paused:boolean;onEdited:(message:PublicChatMessage)=>void;onDeleted:(id:string)=>void}) {
 ...
 if(message.pending||(!own&&!admin)) return null;
 ...
 return <>
  <details className={styles.actions}><summary aria-label={`Actions for message by ${message.author_label}`}>•••</summary><div>
   ...
   <button type="button" onClick={()=>open("delete")}>{own?"Delete":"Delete as admin"}</button>
  </div></details>
  <dialog ref={dialog} aria-labelledby={`message-action-title-${message.id}`} className={styles.dialog} onCancel={event=>{if(busy)event.preventDefault();}}>
   <form onSubmit={event=>{event.preventDefault();void submit();}}>
    <h2 id={`message-action-title-${message.id}`}>{action==="edit"?"Edit message":"Delete message?"}</h2>
    {action==="edit"?<><label htmlFor={`edit-${message.id}`}>Message</label><textarea id={`edit-${message.id}`} value={body} onChange={event=>setBody(event.target.value)} maxLength={600} rows={5} autoFocus onKeyDown={event=>{if(event.key==="Enter"&&!event.shiftKey&&!event.nativeEvent.isComposing){event.preventDefault();void submit();}}}/><small>Enter to save · Shift+Enter for a new line</small></>:<><p>This removes the message from the room and search. This cannot be undone.</p><blockquote>{message.body}</blockquote></>}
