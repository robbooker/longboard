...
type InboxResult = { conversations?: DirectConversation[]; messages?: DirectMessage[]; hasMore?: boolean; hasNewer?: boolean; conversationId?: string; message?: DirectMessage | null };
...
export default function DirectInbox({ controlledConversation,member, target, onTargetClosed, fallbackFocus, sidebarHost, conversationHost, conversationVisible = true, roomSelection = 0, onViewChange }: {
  ...
  const refreshMessages = useCallback(async (id: string) => {
    ...
    let result:InboxResult;
    try{
      // An old context page is not contiguous with the newest page. Refresh its exact IDs
      // until explicit forward pagination closes that gap.
      const ids=hasNewerRef.current?messagesRef.current.map(row=>row.id):[];
      if(ids.length){
    ...
    if(id){
      ...
      void (async()=>{
        ...
        if(id!=='room-summaries'){
          const response=await fetch(`/api/chat/opening?conversation=${id}`,{cache:'no-store'});
          if(!response.ok)throw new Error('Could not find your unread messages. Reopen this conversation to retry.');
          anchor=(await response.json()).messageId;
        }
  ...
  const conversationList = (<aside className={styles.sidebar} aria-label="Private conversations"><h2 className={styles.sectionTitle}>DMs {badge > 0 && <span className={styles.badge} aria-label={`${badge} unread messages or requests`}>{badge}</span>}</h2>
          ...
          {open&&active&&!active.system&&<details className={styles.soundSettings} data-dm-settings><summary>{active.otherName} · conversation settings</summary>
            ...
            {active.status==='accepted'&&!active.unavailable&&!active.blockedByMe&&<ChatPins key={`pin:${member.id}:${active.id}`} memberId={member.id} target={{kind:"dm",conversationId:active.id}} label={active.otherName}/> }
