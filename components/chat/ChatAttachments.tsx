...
export function AttachmentPicker({uploads,disabled=false}:{uploads:ReturnType<typeof useAttachments>;disabled?:boolean}){
 return <div className={styles.drafts}>
  <input ref={uploads.input} type='file' hidden multiple accept='.pdf,.jpg,.jpeg,.png,.gif' aria-label='Attach files' disabled={disabled} onChange={e=>{uploads.addFiles(e.target.files);e.target.value='';}}/>
  ...
  {uploads.files.length>0&&<small>Up to 3 files · 10 MB each · Files are scanned before sharing.</small>}
 </div>;
}
