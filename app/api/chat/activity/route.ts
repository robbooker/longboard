...
export async function GET(req:NextRequest){
 ...
 const result=await db.rpc('chat_activity_inbox',{actor:auth.user.id,rooms:allowedChatRooms(auth.access)});
 return result.error?json({error:'Notifications unavailable.'},503):json(result.data);
}
