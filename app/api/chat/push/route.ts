...
export async function GET(req:NextRequest){const auth=await requireChatUser(req);if(!auth.ok)return json({error:auth.error},auth.status);const config=pushConfiguration();const endpoint=req.nextUrl.searchParams.get('endpoint');let subscribed=false;
 if(endpoint){if(!validPushEndpoint(endpoint))return json({error:'invalid_endpoint'},400);const db=createChatAdminClient();if(!db)return json({error:'push_unavailable'},503);const result=await db.from('chat_push_subscriptions').select('id').eq('account_id',auth.user.id).eq('endpoint',endpoint).maybeSingle();if(result.error)return json({error:'push_unavailable'},503);subscribed=!!result.data;}
 return json({accountId:auth.user.id,configured:!!config,publicKey:config?.publicKey??null,subscribed});}
