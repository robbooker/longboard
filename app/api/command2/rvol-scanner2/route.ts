...
import { enrichHitsWithMonthlyPivots } from "@/lib/scanners/monthlyPivots";
...
export async function GET(request: Request) {
  ...
  try {
    if (resolution !== "all") {
      ...
      const hits = await enrichHitsWithMonthlyPivots(scan.hits, etDate);
    ...
    const hits = await enrichHitsWithMonthlyPivots(combinedHits, etDate);
    const result = {
      ...firstScan,
      fetchedAt: new Date().toISOString(),
      resolution,
