...
export default async function ChatPage({
  ...
  return <PublicChat bootstrap={bootstrap} accountId={auth.user.id} roomRealtime={!auth.serverSession && (room === "social" || (room === "main" && !!auth.access.boardroom))} realtimeRooms={auth.serverSession?[]:rooms.filter(r=>r==="social"||(r==="main"&&auth.access.boardroom))} featureChannel={bootstrap.featureChannel} allowedRooms={rooms} serverSession={auth.serverSession} canLinkShortScout={auth.access.longboard && !auth.access.shortscout} isAdmin={auth.user.role === "admin"} key={auth.user.id} room={room} popout={params.popout === "1"} fontVariableClass={michroma.variable} />;
}
